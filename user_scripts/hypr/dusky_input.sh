#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Dusky Input - Elite Edition v5.4.0 (Engine Sync v3.9.1)
# -----------------------------------------------------------------------------
...
#
# v5.4.0 CHANGELOG:
#   - ENGINE: Full sync with Master v3.9.1 (Hardened).
...
#   - FIX: Float formatting uses proper trailing-zero stripping.
#   - FIX: Input router with Enter/Backspace/Alt+Enter support.
#   - CLEAN: Removed sed dependency and all sed helper functions.
# -----------------------------------------------------------------------------
...
declare -ra TABS=("Keyboard" "Mouse" "Touchpad" "Cursor" "Gestures")
...
render_item_list() {
    ...
    local -i ri
    local item val display config padded_item type
