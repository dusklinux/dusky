#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Kernel Cmdline Editor TUI
# Target: Arch Linux / Limine / Kernel Parameters (/etc/kernel/cmdline)
# Based on Dusky TUI Engine v5.2
# -----------------------------------------------------------------------------
...
register_items() {
    # Kernel Cmdline Layout: register tab_idx "Label" 'key|type|block|min|max|step' "default"
    # Note: 'block' is unused for flat cmdline files; kept for engine compatibility.
    # Cycle values include 'unset' as the first option — selecting it removes the parameter
    # from the cmdline entirely. 'bool' type maps true→bare flag, false→parameter removed.
...
post_write_action() {
    # Triggered automatically after every successful config write.
    declare -g PENDING_BOOTLOADER_UPDATE=1
}
...
# Matches the 'unset' choice in cycle arrays
...
declare -gi PENDING_BOOTLOADER_UPDATE=0
...
cleanup() {
    ...
    # Post-exit prompt: offer to apply changes via limine-update.
    if (( PENDING_BOOTLOADER_UPDATE && TUI_STARTED )) && [[ -t 0 && -t 1 ]]; then
        printf '\n%s[+]%s Changes written to %s\n' "$C_CYAN" "$C_RESET" "$CONFIG_FILE"
        printf '    Run %slimine-update%s to apply to bootloader? [Y/n] ' "$C_WHITE" "$C_RESET"
        local _reply=""
...
# =============================================================================
# KERNEL CMDLINE CACHE PARSER
# =============================================================================

populate_config_cache() {
    local target_path=${WRITE_TARGET:-}
    local cmdline="" token key val
    CONFIG_CACHE=()

    if [[ -z $target_path || ! -f $target_path || ! -r $target_path ]]; then
        # Proceed with empty cache if the file doesn't exist or is unreadable yet.
        return 0
    fi
    ...
    for token in "${tokens[@]}"; do
        if [[ $token == *=* ]]; then
            ...
            # Bare flag (e.g. 'quiet', 'nowatchdog') — treat as boolean true.
...
write_value_to_file() {
    ...
    # Refresh cache UNDER the lock to prevent race conditions
    populate_config_cache
    ...
    current_val=${CONFIG_CACHE[$cache_key]:-$UNSET_MARKER}

    # Skip the write only if the key already exists in the cache with the same value.
    if [[ -n ${CONFIG_CACHE[$cache_key]+_} && $current_val == "$new_val" ]]; then
        release_lock_fd "$lock_fd"
        return 0
    fi

    # Rebuild the cmdline token list from the live file under the lock.
    local cmdline=""
    [[ -f $WRITE_TARGET ]] && cmdline=$(< "$WRITE_TARGET")
    local -a tokens=() new_tokens=()
    read -ra tokens <<< "$cmdline"

    for token in "${tokens[@]}"; do
        local t_key=${token%%=*}
        [[ $t_key == "$target_key" ]] && continue
        new_tokens+=("$token")
    done

    # Append the updated token unless it is being removed.
    if [[ $new_val != "$UNSET_MARKER" && $new_val != "false" ]]; then
        if [[ $new_val == "true" ]]; then
            new_tokens+=("$target_key")
        else
            new_tokens+=("${target_key}=${new_val}")
        fi
    fi

    if ! create_temp_near "$WRITE_TARGET" "mut"; then
        release_lock_fd "$lock_fd"
        set_status "Failed to create staging file."
        return 1
    fi
    scratch=$REPLY
    ...
    if ! create_tmpfile_for_target "$WRITE_TARGET"; then
        remove_temp "$scratch"
        release_lock_fd "$lock_fd"
    ...
    # Update the in-memory cache to reflect the new on-disk state.
    if [[ $new_val == "$UNSET_MARKER" || $new_val == "false" ]]; then
...
draw_main_view() {
    ...
    if [[ -n $STATUS_MESSAGE ]]; then buf+="${C_CYAN} Status: ${C_RED}${STATUS_MESSAGE}${C_RESET}${CLR_EOL}${CLR_EOS}"; else buf+="${C_CYAN} Target: ${C_WHITE}${WRITE_TARGET}${C_RESET}${CLR_EOL}${CLR_EOS}"; fi
    printf '%s' "$buf"
}
...
parse_args() {
    while (($#)); do
        case $1 in
            --help|-h)
                printf 'Usage: %s [--help]\n' "${0##*/}"
                printf '  Interactively edit kernel boot parameters in %s\n' "$CONFIG_FILE"
                printf '  Requires root privileges (will auto-escalate via sudo).\n'
                exit 0
                ;;
            *)
...
main() {
    ...
    # Escalate to root before touching privileged files.
    if (( EUID != 0 )); then
