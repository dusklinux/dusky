...
def test_workspace_recoil_is_v28_stronger_visible_and_non_sizing():
    css = read("style.css")
    assert "v2.8 - status motion corrections" in css
    block = keyframes(css, "workspace-v28-premium-recoil")
    scales = [float(v) for v in re.findall(r"scaleX\((\d+(?:\.\d+)?)\)", block)]
    assert max(scales) >= 1.40
    assert any(0.86 <= value <= 0.90 for value in scales)
...
def test_motion_modes_use_v27_recoil_timing():
    ...
    assert "snapPulseMs: 720" in motion
    assert "snapDelayMs: 70" in motion
    assert "snapPulseMs: 420" in motion
    ...
    assert "workspace-v28-premium-recoil" in soft
    assert "720ms" in soft
    assert "workspace-v28-premium-recoil" in precise
    assert "420ms" in precise
...
def test_clock_reel_css_has_vertical_casino_motion_and_stable_slots():
    ...
    reel = keyframes(css, "clock-v28-reel-new")
